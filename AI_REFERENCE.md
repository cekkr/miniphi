...
- `EfficientLogAnalyzer` now pays attention to `context_requests` from the log-analysis schema, prints the requested snippets/files in the CLI, and stores the hints in `.miniphi/executions/<id>/analysis.json` so operators know exactly what to gather before rerunning the analyzer.
...
8. **P0 – LM Studio transport fallback.** When REST inference hangs or times out, automatically retry via the WS/SDK transport so remote LM Studio hosts don’t stall behind REST timeouts.
9. **P2 – General runtime fixes.** Tackle the "General next steps" list (log analyzer watchdog, decomposer fan-out, schema adapters, etc.) as soon as the P0/P1 stream is quiet.
