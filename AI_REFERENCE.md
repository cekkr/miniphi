...
- Coding-focused presets are available via `--model` or `defaults.model`: `ibm/granite-4-h-tiny` (code-tuned, 16k default ctx) and `mistralai/devstral-small-2507` (code-tuned, ~128k ctx). Phi-4 remains the default unless overridden.
...
13. Harden navigator helper scripts: prefer `python3` (not bare `python`), detect missing interpreters up front, and log helper execution failures in the prompt journal so navigation runs do not stall on missing runtimes.
