...
- During every benchmark run we now copy the Phi-4 prompt transcript out of `.miniphi/recompose/.../prompts.log` into the timestamped run folder as `RUN-###.prompts.log` and surface the relative path via `promptLogExport` inside each `RUN-###.json`, so reviewers can audit the exact LM Studio prompts/responses without chasing hidden workspace files.
