...
- Default workflow (`node src/index.js run --cmd "npm test" --task "Analyze failures"`) executes a command, compresses stdout/stderr, and streams Granite reasoning in real time (requires LM Studio at `http://127.0.0.1:1234` with `ibm/granite-4-h-tiny`; swap to `microsoft/phi-4-reasoning-plus` or `mistralai/devstral-small-2507` via `--model` when needed).
- Model presets are available via `--model` or `defaults.model`: default `ibm/granite-4-h-tiny` (code-tuned, 16k default ctx), `microsoft/phi-4-reasoning-plus` (general-purpose, 32k default ctx/131k max), and `mistralai/devstral-small-2507` (code-tuned, ~128k ctx).
