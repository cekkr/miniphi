...
2. **P0 – LM Studio health gates.** Cache `/api/v0/status`, GPU utilization, and tokenizer headroom before dispatching prompts so archived journals explain outages (Project-wide Immediate #2).
3. **P0 – Observability + logging.** Keep attaching request IDs, schema ids, and navigation/helper context to stdout plus `.miniphi/dev-logs/` for reproducible triage (Project-wide Immediate #3).
4. **P0 – Prompt template baselines.** Treat truncation scaffolds as runtime assets: capture them under `.miniphi/prompt-exchanges/templates/`, expose the CLI helper described below, and make sure Phi’s JSON schema + `truncation_strategy` directives stay synchronized across docs/code.
5. **P1 – Documentation + onboarding.** Sync README/AI_REFERENCE/CLI help and publish the operator playbook once runtime toggles settle (Project-wide Immediate #4).
6. **P1 – Benchmark & regression loops.** Keep the besh timeout journal pinned, re-run it whenever timeout/logging stacks evolve, and treat failures as regressions (Project-wide Immediate #5).
7. **P1 – Helper/script lifecycle.** Wire helpers back into prompt journals, propagate timeout enforcement, and make helper replays observable (Project-wide Immediate #6).
8. **P2 – General runtime fixes.** Tackle the "General next steps" list (log analyzer watchdog, decomposer fan-out, schema adapters, etc.) as soon as the P0/P1 stream is quiet.
...
- Phi-4's `truncation_strategy` responses no longer disappear: every run stores the plan under `.miniphi/executions/<id>/truncation-plan.json`, and `node src/index.js analyze-file ... --resume-truncation <id> [--truncation-chunk <selector>]` replays the suggested chunk (including the requested line window) so follow-up prompts stop re-reading the entire dataset.
- Recommended remediation commands are now harvested automatically: whenever Phi populates `recommended_fixes[].commands`, MiniPhi records the normalized commands (plus descriptions/files) under `.miniphi/helpers/command-library.json` so subsequent runs, helpers, or operators can replay battle-tested fixes.
...
- Build on the new truncation-plan pipeline: `analyze-file` already persists Phi’s `truncation_strategy` under `.miniphi/executions/<id>/truncation-plan.json` and exposes `--resume-truncation <id>` plus `--truncation-chunk <selector>` to focus the recommended slice, but we still need to auto-run helper commands/scripts from the plan, mark completed chunks, and chain the remaining prompts without manual re-invocation.
...
- Extend `docs/prompts/log-analysis.schema.json` with an optional `context_requests` array so Phi can explicitly ask for missing snippets, external descriptions, or deeper dives instead of MiniPhi sending the entire dataset by default. Wire this field into the CLI so the operator sees the request and can provide the strictly needed additions—lowering both API call count and token usage—before rerunning besh-sized prompts (`.miniphi/executions/4354cd51-a6ce-488e-a2f3-eff725aa61af/prompt.json:1-90` currently burns context repeating the full schema and severity table just to time out again).
...
2. **LM Studio health gates.** Before dispatching Phi, cache `/api/v0/status`, GPU utilization, and tokenizer headroom; stash those snapshots next to `.miniphi/executions/<id>/` so archived journals explain whether a timeout came from LM Studio saturation, CLI policy, or workspace constraints.
...
4. **Documentation + onboarding.** Sync `README.md`, `AI_REFERENCE.md`, and CLI `--help` with every runtime toggle that touches orchestration (timeouts, schema adapters, journal policies). Ship a short “operator playbook” that walks through configuring LM Studio, verifying local prompts, and interpreting the verbose logs.
5. **Benchmark & regression loops.** Promote the besh prompt journal timeout into a standing regression under `benchmark analyze`, pin its `.miniphi/executions` bundle to git LFS (or another artifact store), and rerun it whenever the Phi timeout/logging stack changes so we can catch regressions before they land in `main`.
6. **Helper/script lifecycle.** Track helper scripts generated under `.miniphi/helpers/` back into prompt journals, propagate the new timeout controls into helper execution, and let operators replay helpers with verbose logging enabled so we can profile the full orchestration stack, not just the Phi segment.
