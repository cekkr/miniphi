...
- Teach `analyze-file` to persist and consume the `truncation_strategy` block when Phi falls back. The failing run already marked `{\"should_split\":true,\"chunking_plan\":[...]}` inside `.miniphi/executions/a372e869-f2f3-4c5c-afa7-0ab74458cc20/analysis.json:18`, but the CLI ignored it; store that JSON under `.miniphi/executions/<id>/truncation-plan.json`, expose a `--resume-truncation <id>` helper, and spawn follow-up chunk prompts automatically so the API’s own splitting guidance produces real work.
