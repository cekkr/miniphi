...
- **Recursive task planning.** `src/libs/prompt-decomposer.js` preflights each CLI invocation through LM Studio’s REST API, generates a JSON plan of sub-prompts/actions, and persists that plan under `.miniphi/prompt-exchanges/decompositions/` so later runs can resume or inspect the tree.
