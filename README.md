...
Use `--model <id>` (or set `defaults.model`) to switch from the default Phi-4 to coding presets like `ibm/granite-4-h-tiny` (16k ctx) or `mistralai/devstral-small-2507` (~128k ctx); MiniPhi clamps the context length to each preset unless you override it.
