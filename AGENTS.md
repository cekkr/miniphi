...
- The interactive agent turn schema is `agent-action` (`actions[]` with `type` ∈ read_file/list_dir/search_text/web_research/write_file/edit_file/run_cmd/finish, plus `needs_more_context`/`missing_snippets`); `AgentSession` validates every turn, retries once with a compact nudge, and otherwise falls back to a deterministic `finish` with a stop reason (`invalid-response`, `session-timeout`, `no-actions`, `no-progress`, `max-turns`, `cancelled`).
