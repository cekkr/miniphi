...
- `src/libs/cheetah-context-engine.js` implements the bounded one-line TCP client plus `CheetahContextEngine`. Enable it with `context.engine: "cheetah"` (or `MINIPHI_CONTEXT_ENGINE=cheetah`); connection settings live under `context.cheetah` and default to loopback port `4455`, a project-derived database, a 2500 ms timeout, and `required: false`.
...
- The adapter mirrors stable project/session-scoped node ids, layer/label/state/importance metadata, model-authored relations, mission membership, and subtask membership. It never mirrors full context text or the workspace path. Edge writes use idempotent `GRAPH_EDGE_SET_BATCH`; locally dropped/superseded nodes are removed with `GRAPH_NODE_DEL cascade=1`. Failed or partial batches clear local fingerprints so the whole metadata set is replayed on the next bounded attempt.
- Before every interactive agent prompt or focused subtask prompt, `AgentSession` calls `GRAPH_RECALL` with the mission and current focus as seeds. Returned local node ids receive a selection-only score boost; Cheetah cannot mutate durable importance, bypass retained layers, or bypass the token budget.
- Optional failures fall back to the in-memory selector after a cooldown. `required: true` converts a Cheetah failure into the turn's deterministic JSON stop fallback. Engine selection/fallback/mirror/recall telemetry persists in `.miniphi/agent-sessions/<id>/context-engine.json` and under `result.json.context.engine`.
...
- UI routing (`src/ui/route.js` `decideUiLaunch`): on a TTY, bare `miniphi`, the `ui` command, and a free-form task open the Ink UI; `--headless`/`--no-ui`, a non-TTY (scripts/CI), and every explicit subcommand run headless. This keeps direct arguments always available while the UI is the primary interface. The UI path (`launchInteractiveUi` in `src/index.js`) scans native v1, asks for the task, then presents `Auto` plus manual model choices before starting; the resolved model/load snapshot is persisted in session/result JSON. It builds its own lightweight REST client (never a static `context_length`) and dynamic-imports `src/ui/launch.js` so Ink/React never load on headless runs.
...
- `node --test unit-tests-js/cheetah-context-engine.test.js unit-tests-js/context-graph.test.js unit-tests-js/agent-context-layers.test.js` to validate Cheetah protocol framing, metadata mirroring, recall-driven query boosts, fallback, and session telemetry without live services.
- With Cheetah and LM Studio running: `MINIPHI_LMSTUDIO_INTEGRATION=1 MINIPHI_CHEETAH_INTEGRATION=1 LMSTUDIO_REST_URL=http://127.0.0.1:1234 MINIPHI_LIVE_MODEL=qwen2.5-coder-7b-instruct node --test unit-tests-js/agent-cheetah-context.live.test.js`.
...
- `src/libs/cheetah-context-engine.js`: Optional Cheetah TCP adapter — config resolution, metadata mirroring, bounded `GRAPH_RECALL`, query-time preferred node ids, fallback, and telemetry.
...
- **Layered context / Cheetah:** [`context-graph.test.js`](unit-tests-js/context-graph.test.js), [`agent-context-layers.test.js`](unit-tests-js/agent-context-layers.test.js), [`cheetah-context-engine.test.js`](unit-tests-js/cheetah-context-engine.test.js).
