...
- `src/libs/cheetah-context-engine.js` implements the bounded one-line TCP client plus `CheetahContextEngine`. Enable it with `context.engine: "cheetah"` (or `MINIPHI_CONTEXT_ENGINE=cheetah`); connection settings live under `context.cheetah` and default to loopback port `4455`, database `miniphi_context`, a 2500 ms timeout, and `required: false`.
- The adapter mirrors stable session-scoped node ids, layer/label/state/importance metadata, model-authored relations, mission membership, and subtask membership. It never mirrors full context text. Edge writes use idempotent `GRAPH_EDGE_SET_BATCH`; failed or partial batches clear local fingerprints so the whole metadata set is replayed on the next bounded attempt.
...
- Current scope is the interactive `AgentSession` path (`ui` and its model-opened sub-conversations). Headless `run`/`workspace` decomposer integration, remote deletion/compaction, and cross-session recall are not implemented yet; do not claim they use Cheetah.
...
- CheetahContextEngine (`src/libs/cheetah-context-engine.js`): opt-in metadata mirror and `GRAPH_RECALL` query adapter over Cheetah's bounded one-line TCP protocol. `createCheetahContextEngineFactory` wires config/env settings into interactive sessions; telemetry persists separately from the canonical local graph.
...
- **Live-gated tier (`MINIPHI_LMSTUDIO_INTEGRATION=1`):** [`lmstudio-code-generation.live.test.js`](unit-tests-js/lmstudio-code-generation.live.test.js) (executes generated code), [`agent-cheetah-context.live.test.js`](unit-tests-js/agent-cheetah-context.live.test.js) (also requires `MINIPHI_CHEETAH_INTEGRATION=1`), [`cli-bash-advanced.test.js`](unit-tests-js/cli-bash-advanced.test.js), [`benchmark-general-live-lm.test.js`](unit-tests-js/benchmark-general-live-lm.test.js).
