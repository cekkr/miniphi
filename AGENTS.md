...
- CLI entrypoints: `ui` (interactive agent, also the default for bare `miniphi` / free-form tasks on a TTY), `run`, `analyze-file`, `workspace` (`miniphi "<task>" --headless`), `recompose`, `benchmark models|general|recompose|analyze|plan scaffold`, `cache-prune`, `migrate-stop-reasons`, `lmstudio-health`, `web-browse`, `nitpick`, `cheetah-learn teach|ask|chat|questions|eval` (see "Cheetah-learn" below), and helper/command-library browsers.
...
Delivered 2026-07-28 — Cheetah-learn: ignorant-model teach/recall over world knowledge (live-proven):
- New `cheetah-learn teach|ask|chat|questions|eval` command (`src/commands/cheetah-learn.js`) drives
  a deliberately small, "ignorant" model (fixed to `qwen2.5-coder-0.5b-instruct`, a distinct LM
  Studio host via `--base-url`/`--host`) that never answers facts from its own parameters: it
  **teaches** Cheetah what it reads (e.g. `rahular/simple-wikipedia` via the new
  ...
  adapter (reuses `CheetahTcpClient` from `cheetah-context-engine.js` unmodified; a distinct database
  `miniphi_knowledge`); `src/libs/cheetah-learner.js` orchestrates with nitpick.js's exact
  double-layer schema-retry-then-deterministic-fallback pattern. No Cheetah Go source needed
  touching — every primitive used (`GRAPH_EDGE_SET_BATCH`, confidence/modality, `GRAPH_RECALL` with
  `references=1`, cursor-pageable `GRAPH_NEIGHBORS`) was already shipped and tested.
- **Load-bearing anti-hallucination rule**: groundedness is adjudicated by the adapter, never taken
  from the model's own `grounded` field alone — `effectiveGrounded = anchorResult.resolved &&
  parsed.grounded === true`. Live-proven twice: two `cheetah-learn eval` runs taught 30 real Simple
...
- Regression: `node --test unit-tests-js/hf-dataset-client.test.js
  ...
  unit-tests-js/agent-executor.test.js unit-tests-js/agent-session.test.js` (51 tests across the
  cheetah-learn-specific files, offline, fake TCP-client/REST-client objects, same fixture style as
...
- CheetahKnowledgeClient/CheetahLearner (`src/libs/cheetah-knowledge-client.js`, `src/libs/cheetah-learner.js`): the "ignorant model" teach/recall adapter behind `cheetah-learn` (see "Cheetah-learn" below) and behind the interactive agent's optional `knowledge_lookup` action above. Built on the same reused `CheetahTcpClient` as CheetahContextEngine, but a distinct database (`miniphi_knowledge` by default) and data model: subjects anchor at type-agnostic `topic:<slug(name)>` ids (the model's chosen type becomes a label, not part of the id) so a name alone re-derives the same anchor at question time; open questions reuse the graph's own `agent -[:unsure_about]-> hypothesis:<slug>` shape with resolution flipping `props.status` open→resolved rather than deleting the edge. `resolveKnowledgeLookupConfig()`/`createKnowledgeLookupAction()` compose the reachability-gated agent action; `HfDatasetClient`/`fetchDisjointSamples` (`src/libs/hf-dataset-client.js`) page a HuggingFace dataset via the free `datasets-server.huggingface.co` JSON API (no Python/parquet) for teach/eval sampling; `CheetahRunReport` (`src/libs/cheetah-run-report.js`) is a lightweight `.miniphi/cheetah/runs/` JSON log, separate from `MiniPhiMemory` since the knowledge of record lives in Cheetah itself.
...
- `node --test unit-tests-js/hf-dataset-client.test.js unit-tests-js/cheetah-knowledge-client.test.js unit-tests-js/cheetah-learner.test.js unit-tests-js/cheetah-run-report.test.js unit-tests-js/cheetah-learn-command.test.js` to validate the cheetah-learn adapter/orchestration/CLI layer offline (fake TCP/REST clients, no live services): id/prop encoding, the teach/recall ladders, the anti-hallucination groundedness override, open-question record/resolve/list, and the CLI's teach/ask/questions dispatch plus `--no-save`.
...
- `src/libs/cheetah-context-engine.js`: Optional Cheetah TCP adapter — config resolution, metadata + complete-reference-sentence mirroring, bounded `GRAPH_RECALL references=1`, query-time preferred node ids/reference candidates, fallback, and telemetry.
