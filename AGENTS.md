...
- `node --test unit-tests-js/hf-dataset-client.test.js unit-tests-js/local-wikipedia-dataset.test.js unit-tests-js/cheetah-wikipedia-runner.test.js unit-tests-js/cheetah-knowledge-client.test.js unit-tests-js/cheetah-learner.test.js unit-tests-js/cheetah-run-report.test.js unit-tests-js/cheetah-learn-command.test.js` to validate the cheetah-learn adapter/orchestration/CLI layer offline (fake TCP/REST clients, no live services): streamed local shards and resume, bounded stop reasons, fixed-subject probes, Unicode-safe writes, source-grounded teach/recall, deterministic reference fallback, open-question record/resolve/list, and CLI dispatch plus `--no-save`.
