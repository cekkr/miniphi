...
- `docs/prompts/*.schema.json` are the schema source of truth (including `agent-action`, `context-ops`, `visual-review`, `nitpick-plan`, `nitpick-research-plan`, `nitpick-draft`, `nitpick-critique`); cached templates live under `.miniphi/prompt-exchanges/templates/`.
- The interactive agent turn schema is `agent-action` (`actions[]` with `type` ∈ read_file/list_dir/search_text/web_research/visual_review/write_file/edit_file/run_cmd/finish, plus `needs_more_context`/`missing_snippets` and the optional context-graph fields `context_ops`/`context_sufficient`/`context_gap`); `AgentSession` validates every turn, retries once with a compact nudge, and otherwise falls back to a deterministic `finish` with a stop reason (`invalid-response`, `session-timeout`, `no-actions`, `no-progress`, `max-turns`, `cancelled`). `visual_review`'s own reply (from a separate vision-capable model, not the coding model) is schema-bound separately by `visual-review` (see VisionReviewer above).
