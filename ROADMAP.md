...
1) Core loop hardening
   ...
   - Stop-reason persistence closeout (2026-02-10):
     ...
     - Added one-shot migration command `migrate-stop-reasons` to normalize historical stop-reason aliases in already-written `.miniphi/**/*.json` artifacts (supports `--dry-run`, `--json`, `--strict`, `--parse-error-report`, and `--include-global`).
     - Regression coverage added: `unit-tests-js/miniphi-memory-stop-reason.test.js`, `unit-tests-js/task-execution-register-stop-reason.test.js`, `unit-tests-js/cli-implicit-run.test.js`, `unit-tests-js/stop-reason-migrator.test.js`, `unit-tests-js/cli-migrate-stop-reasons.test.js` plus updates to `unit-tests-js/lmstudio-error-utils.test.js`, `unit-tests-js/prompt-recorder.test.js`, and `unit-tests-js/prompt-step-journal.test.js`; `npm test` passed (`69/69`).
   - Live persistence proof runs (2026-02-10):
     ...
     - `node src/index.js migrate-stop-reasons --history-root . --dry-run --strict --parse-error-report --json` fail-fasted on malformed historical JSON and emitted direct malformed path entries in `parseErrorFiles`.
