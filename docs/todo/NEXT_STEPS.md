...
- [ ] Detect "no workspace provided" replies in `workspace-overview` prompts (see `clean-roundtrip.prompts.log` lines 25-920) and automatically resend with the manifest + README excerpt so every prompt execution starts with real context instead of wasting several minutes on boilerplate.
- [ ] Cache code-to-markdown narratives per file hash inside `MiniPhiMemory` so repeated hello-flow-plan sweeps reuse the previous 9 descriptions; the latest plan spent 1.0–1.3M ms per sweep for unchanged sources (`SUMMARY.md` lines 16-25).
- [ ] Persist benchmark summary stats (phase durations, warnings, mismatches) into `.miniphi/history/benchmarks.json` and auto-attach TODO links so project managers do not have to manually copy information like the four "missing code block" warnings from `clean-roundtrip.json`.
